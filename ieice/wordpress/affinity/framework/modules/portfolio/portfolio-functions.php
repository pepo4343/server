...
if(!function_exists('affinity_mikado_single_portfolio')) {
	/**
	 * Loads holder template for portfolio single
	 */
	function affinity_mikado_single_portfolio() {
		$portfolio_template = affinity_mikado_get_portfolio_single_type();
		...
		if($portfolio_template == 'gallery') {
			$params['holder_class'][] = 'mkd-portfolio-gallery-'.affinity_mikado_options()->getOptionValue('portfolio_single_numb_columns');
		}

		affinity_mikado_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
	}
}

if(!function_exists('affinity_mikado_get_portfolio_single_type')) {
	/**
	 * Returns template of current portfolio item
	 *
	 * @return bool|mixed|void
	 */
	function affinity_mikado_get_portfolio_single_type() {
		return affinity_mikado_get_meta_field_intersect('portfolio_single_template');
	}
}

if(!function_exists('affinity_mikado_get_portfolio_single_media')) {
	/**
	 * Returns a media array of current portfolio item
	 *
	 * @return array
	 */
	function affinity_mikado_get_portfolio_single_media() {
		...
		if(is_array($videos) && count($videos)) {
			usort($videos, 'affinity_mikado_compare_portfolio_videos');
			foreach($videos as $video) {
				...
				if(!empty($video['portfoliovideotype'])) {
					...
					$media['video_url']   = affinity_mikado_portfolio_get_video_url($video);
...
if(!function_exists('affinity_mikado_portfolio_get_video_url')) {
	/**
	 ...
	 * @return array|string
	 */
	function affinity_mikado_portfolio_get_video_url($video) {
...
if(!function_exists('affinity_mikado_portfolio_get_media_html')) {
	/**
	 * Loads template for current portfolio item media
	 *
	 * @param $media
	 */
	function affinity_mikado_portfolio_get_media_html($media) {
		global $wp_filesystem;
		$params = array();

		//For adding image overlay in gallery template type
		$params['gallery'] = (affinity_mikado_get_portfolio_single_type() == 'gallery') ? true : false;

		if($media['type'] == 'image') {
			$params['lightbox'] = affinity_mikado_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
		...
		if(in_array($media['type'], array('youtube', 'vimeo'))) {
			$params['lightbox'] = affinity_mikado_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
		...
		affinity_mikado_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
	}
}

if(!function_exists('affinity_mikado_compare_portfolio_videos')) {
	/**
	 ...
	 */
	function affinity_mikado_compare_portfolio_videos($a, $b) {
...
if(!function_exists('affinity_mikado_compare_portfolio_options')) {
	/**
	 ...
	 * @return int result of comparison
	 */
	function affinity_mikado_compare_portfolio_options($a, $b) {
...
if(!function_exists('affinity_mikado_portfolio_get_info_part')) {
	/**
	 * Loads portfolio info item based on passed param
	 *
	 * @param $part
	 */
	function affinity_mikado_portfolio_get_info_part($part) {
		$portfolio_template = affinity_mikado_get_portfolio_single_type();

		affinity_mikado_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template);
	}
}

if(!function_exists('affinity_mikado_portfolio_get_single_navigation')) {
	/**
	 *
	 */
	function affinity_mikado_portfolio_get_single_navigation() {
		$params = array();

		$in_same_term = affinity_mikado_options()->getOptionValue('portfolio_single_nav_same_category') == 'yes';
		...
		affinity_mikado_get_module_template_part('templates/single/parts/navigation', 'portfolio', '', $params);
	}
}